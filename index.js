...
function applyAutomaticPerformanceProfile() {
  const os = require('os');
  const cpuCount = Array.isArray(os.cpus()) ? os.cpus().length : 2;
  const totalMemMb = Math.max(512, Math.round(os.totalmem() / 1024 / 1024));

  const dynamicCeil = Math.max(8, Math.min(48, cpuCount * 3));
  const dynamicFloor = Math.max(4, Math.min(dynamicCeil - 2, Math.ceil(dynamicCeil * 0.45)));
  const dynamicStart = Math.max(dynamicFloor, Math.min(dynamicCeil, Math.ceil(dynamicCeil * 0.8)));
  const dynamicDelay = totalMemMb >= 8192 ? 20 : (totalMemMb >= 4096 ? 35 : 50);
  const dynamicSoftMem = Math.max(1024, Math.floor(totalMemMb * 0.72));

  bootstrapConfig.maxParallelSubBotBoot = dynamicStart;
  bootstrapConfig.subBotBootDelayMs = dynamicDelay;

  config.performance = config.performance || {};
  config.performance.maxParallelSubBotBootFloor = dynamicFloor;
  config.performance.maxParallelSubBotBootCeil = dynamicCeil;
  config.performance.memorySoftLimitMb = dynamicSoftMem;
}
...
client.once('ready', () => {
  applyAutomaticPerformanceProfile();
  runtimePreflightChecks();
  console.log(`Logged in as ${client.user.tag}`);
  setInterval(() => { checkSubscriptions().catch(() => {}); }, 30000);
  setInterval(() => { autoTunePerformance(); }, Math.max(5000, Number(config?.performance?.autoTuneIntervalMs ?? 15000)));
});
