const { PermissionsBitField, GatewayIntentBits, Partials, Client, EmbedBuilder, AttachmentBuilder, Collection, ActionRowBuilder, ButtonBuilder, ButtonStyle, WebhookClient, ActivityType, ChannelType } = require('discord.js');
...
const storeOwnerId = Array.isArray(owners) && owners.length > 0 ? owners[0] : null;
...
function buildOwnerFooterEmbed(ownerId) {
  const targetOwnerId = ownerId || storeOwnerId;
    ...
    logsArray.forEach((log, index) => {
      ...
      if (remainingTime <= 0) {
        ...
        if (user) {
          ...
          user.send( {
            files: ['https://cdn.discordapp.com/attachments/1200282105469485146/1224545663732416572/2_days.png?ex=661de205&is=660b6d05&hm=7e492e68b6c46ad87b7e667c3128f6b579e97012f1393bf36083bac17f4819a2&'],
          });
...
async function runBotSystem(token) {
  ...
  function getGuildQueue(guildId) {
    ...
    if (!q) {
      q = { player: null, tracks: [], trackMessages: [], current: null, currentMessage: null, loop: false, volume: 50, paused: false, joining: false };
  ...
  async function ensurePlayer(guild, channelId) {
    const node = client83883.shoukaku.getIdealNode();
  ...
  .setEmoji("<:next:1223341791437258852>")
  .setStyle(ButtonStyle.Secondary);
const volumeUpButton = new ButtonBuilder()
  .setCustomId('volumeUpButton')
  .setEmoji("<:volume1:1223341703302483979>")
  .setStyle(ButtonStyle.Secondary);
  const stopButton = new ButtonBuilder()
  .setCustomId('pauseButton')
  .setEmoji("<:pause:1223341686210695209>")  // تغيير الإيموجي للإشارة للإيقاف المؤقت
  .setStyle(ButtonStyle.Secondary);
  const volumeDownButton = new ButtonBuilder()
  .setCustomId('volumeDownButton')
  .setEmoji("<:volumedown:1223341754695286835>")
  .setStyle(ButtonStyle.Secondary);
  const repeatButton = new ButtonBuilder()
  .setCustomId('repeatButton')
  .setEmoji("<:arrowcircle1211:1225426491387674674>")
  .setStyle(ButtonStyle.Secondary);
const row = new ActionRowBuilder()
    ...
    switch (interaction.customId) {
      ...
      case 'volumeUpButton':
        ...
        if (newVolumeUp <= 150) {
          ...
          queue.volume = 150;
          await playerSetVolume(queue.player, 150);
  ...
  client83883.on('ready', async () => {
    ...
    // Default presence based on stored status (streaming/server name if not set)
